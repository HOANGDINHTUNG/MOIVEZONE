import { Link } from "react-router-dom";
import type { TvNetworkSummary } from "../pages/DetailsPage";
import Divider from "../../../components/common/ux/Divider";

type DetailsNetworksSectionProps = {
  networks: TvNetworkSummary[];
  imageURL: string; // TMDB base URL
};

const DetailsNetworksSection: React.FC<DetailsNetworksSectionProps> = ({
  networks,
  imageURL,
}) => {
  if (!networks.length) return null;

  return (
    <>
      <Divider />
      <section className="mt-6 px-1 sm:px-0">
        <h2 className="text-[13px] sm:text-sm md:text-base font-semibold uppercase tracking-wide text-neutral-200">
          Networks phát hành
        </h2>

        <div className="mt-4 flex flex-wrap gap-3 sm:gap-4 md:gap-5">
          {networks.map((network) => (
            <Link
              key={network.id}
              to={`/network/${network.id}`}
              className="group relative flex h-14 w-28 items-center justify-center rounded-xl
           border border-neutral-800 bg-neutral-950/70 backdrop-blur-sm
           shadow-[0_0_20px_rgba(0,0,0,0.4)]
           transition-transform duration-200 hover:scale-[1.05]
           hover:border-red-500/80 hover:shadow-[0_0_35px_rgba(255,60,60,0.35)]
           overflow-hidden sm:h-16 sm:w-32"
            >
              {/* Glow hiệu ứng */}
              <div
                className="pointer-events-none absolute inset-0 opacity-0 group-hover:opacity-40 transition
                              bg-[radial-gradient(circle_at_center,rgba(255,70,70,0.25),transparent_70%)]"
              />

              <div className="flex h-full w-full items-center justify-center p-3">
                {network.logo_path ? (
                  <img
                    src={`${imageURL}/w500${network.logo_path}`}
                    alt={network.name}
                    className="max-h-8 w-full object-contain transition-all
                                sm:max-h-10 group-hover:scale-110 group-hover:brightness-125"
                  />
                ) : (
                  <span className="text-[10px] sm:text-xs text-neutral-400">
                    No Logo
                  </span>
                )}
              </div>

              {/* ring border khi hover */}
              <div
                className="pointer-events-none absolute inset-0 rounded-xl 
                              ring-0 ring-red-500/40 transition-all group-hover:ring-2"
              />
            </Link>
          ))}
        </div>
      </section>
    </>
  );
};

export default DetailsNetworksSection;
